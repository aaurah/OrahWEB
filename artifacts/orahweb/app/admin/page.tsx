        ...
        {KPI.map(({ label, value, delta, up, sub }) => (
          <div key={label} className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
            ...
            <div className="mt-2 flex items-center gap-1.5">
              <span className={`text-xs font-semibold px-1.5 py-0.5 rounded ${up ? "bg-emerald-900/50 text-emerald-400" : "bg-red-900/30 text-red-400"}`}>
                {up ? "▲" : "▼"} {delta}
              </span>
        ...
        <div className="xl:col-span-2 bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-gray-800">
            {RECENT_DOMAINS.map((d) => (
              <div key={d.domain} className="flex items-center justify-between px-6 py-3.5">
                <div>
                  <p className="text-sm font-semibold text-white">{d.domain}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{d.user}</p>
                </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-gray-800">
            {SYSTEM_STATUS.map(({ service, status, uptime }) => (
              <div key={service} className="px-6 py-3.5 flex items-center justify-between">
