    ...
    <div className="p-4 sm:p-8 space-y-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400 bg-gray-800 px-2.5 py-1.5 rounded-lg shrink-0">
          <svg className="w-3 h-3 text-emerald-400" fill="currentColor" viewBox="0 0 8 8"><circle cx="4" cy="4" r="3" /></svg>
          Live · PostgreSQL
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4">
        {[
          ...
          { label: "Banned", value: users.filter((u) => u.role === "banned").length },
        ].map(({ label, value }) => (
          <div key={label} className="bg-gray-900 border border-gray-800 rounded-2xl p-4 sm:p-5 text-center">
            <p className="text-2xl sm:text-3xl font-bold text-white">{loading ? "—" : value}</p>
            <p className="text-[10px] sm:text-xs text-gray-500 mt-1 uppercase tracking-wider font-semibold">{label}</p>
          </div>
        ))}
      </div>
              ...
              <thead>
                <tr className="bg-gray-800/50 border-b border-gray-800">
                  {["User", "Role", "Joined", "Last Login", "Status", "Actions"].map((h) => (
                    <th key={h} className="px-4 sm:px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-800">
                {filtered.map((u) => (
                  <tr key={u.id} className="hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="px-4 sm:px-5 py-4 text-gray-400 whitespace-nowrap text-xs sm:text-sm">{u.joined}</td>
                    <td className="px-4 sm:px-5 py-4 text-gray-400 whitespace-nowrap text-xs sm:text-sm">{formatLastLogin(u.last_login)}</td>
                    <td className="px-4 sm:px-5 py-4">
                      {u.verified ? (
